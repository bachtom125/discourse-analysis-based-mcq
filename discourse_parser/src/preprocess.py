...
def get_edus_from_file(fedu):
    """Get EDUs from .edu file and return a list of EDUs
    """
    edus = []
    try: 
        with open(fedu, 'r') as file:
            for line in file:
                if not line.strip():
                    continue
...
def arg_parse():
    ...
    parser.add_argument('--data_dir', required=False, help='path to data directory.', default='../data/sample')
    parser.add_argument('--corenlp_dir', required=False, help='path to Stanford Corenlp directory.', default='./CoreNLP/')
