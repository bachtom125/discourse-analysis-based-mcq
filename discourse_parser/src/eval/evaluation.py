...
class Evaluator(object):
    ...
    def eval_parser(self, path='./examples', report=False, bcvocab=None, draw=True):
        ...
        for fmerge in doclist:
            ...
            fpickle = fmerge.replace('.merge', '.pickle') # segment pairs to serve as input to relation labeller
            pred_rst.export_tree(fpickle)
            ...
            # Get RST tree visualization
            rst_tree = pred_rst.get_rst_tree()
            # Write RST tree intro file
            ftree = fmerge.replace('.merge', '.tree')
            with open(ftree, 'w') as ftree_file:
                ftree_file.write(rst_tree.show(stdout=False, sorting=False))
                print("\nWritten to ", ftree, " successfully!\n")
